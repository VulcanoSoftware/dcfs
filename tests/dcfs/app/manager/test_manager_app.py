...
class TestManagerApp:
    ...
    def mock_config(self, mocker):
        ...
        config.discord.private_file_channel = ["123456"]
        config.dcfs = mocker.Mock()
        channel = mocker.Mock()
        channel.name = "Test-Channel"
        config.dcfs.metadata = {"123456": channel}
        return config
    ...
    async def test_get_tasks_all(self, manager_app, mocker):
        mock_task_store = mocker.patch("dcfs.app.manager.app.task_store")
        mock_tasks = [
    ...
    @pytest.mark.asyncio
    async def test_get_discord_message_success(
    ...
    @pytest.mark.asyncio
    async def test_get_discord_message_wrong_channel(self, mock_clients, mock_config):
        app = create_manager_app(mock_clients, mock_config)
        client = TestClient(app)
    ...
    @pytest.mark.asyncio
    async def test_get_discord_message_not_found(
    ...
    @pytest.mark.asyncio
    async def test_get_discord_message_no_document(
        self, mock_client, mock_clients, mock_config, mocker
    ):
        mock_message = mocker.Mock()
        mock_message.message_id = 456
    ...
    @pytest.mark.asyncio
    async def test_import_discord_message_success(
        self, mock_client, mock_clients, mock_config, mocker
    ):
        # Mock the message response
        mock_gfc = mocker.patch("dcfs.app.manager.app.gfc")
        mock_channel_cache = mock_gfc["Test-Channel"]
        mock_ops_class = mocker.patch("dcfs.app.manager.app.Ops")
        mock_message = mocker.Mock()
        mock_message.message_id = 456
        mock_message.document = mocker.Mock(size=1024, mime_type="text/plain")
    ...
    @pytest.mark.asyncio
    async def test_import_discord_message_wrong_channel(
