...
from dcfs.core.api.metadata import MetaDataApi
...
class TestFileApi:
    ...
    @pytest.fixture
    def sample_directory(self) -> DCFSDirectory:
        return DCFSDirectory.root_dir()

    @pytest.fixture
    def sample_file_ref(self, sample_directory) -> DCFSFileRef:
        return DCFSFileRef(
            message_id=123, name="test_file.txt", location=sample_directory
        )

    @pytest.fixture
    def sample_file_desc(self, mocker) -> DCFSFileDesc:
        # Create a mock DCFSFileDesc with necessary attributes
        file_desc = mocker.Mock(spec=DCFSFileDesc)
        file_desc.get_latest_version.return_value = DCFSFileVersion(
            id="v1",
    ...
    async def test_create_new_file(
        ...
        mock_response.fd = mocker.Mock(spec=DCFSFileDesc)
    ...
    async def test_rm_without_version_id(
        ...
        fd = mocker.Mock(spec=DCFSFileDesc)
    ...
    async def test_rm_with_version_id(
        ...
        fd = mocker.Mock(spec=DCFSFileDesc)
        fd.get_version.return_value = DCFSFileVersion(
            id=version_id,
            updated_at=datetime.datetime.now(),
            message_ids=[500, 501],
        )
