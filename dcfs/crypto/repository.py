...
from dcfs.crypto.header import HEADER_SIZE, MAGIC, FileHeader, InvalidHeaderError
...
class EncryptingFileContentRepository(IFileContentRepository):
    ...
    async def content_length(self, fv: "DCFSFileVersion") -> int:
        ...
        try:
            ...
            logger.warning(f"Could not verify encryption header for version {fv.id}: {ex}")
            return fv.size
