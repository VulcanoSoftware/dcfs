...
export default function FileExplorer({
  ...
  const [discordLinkDialog, setDiscordLinkDialog] = useState(false);
  ...
  const handleDiscordImport = useCallback(
    async (channelId: number, messageId: number, asName: string) => {
      if (!messageId || !currentPath) return;

      await managerClient.importDiscordMessage(
        channelId,
        messageId,
        currentPath,
        asName
      );
      setDiscordLinkDialog(false);
      await loadDirectory(currentPath);
      setSnackbar({
        open: true,
  ...
  return (
    ...
    >
      ...
      <ImportDialog
        open={discordLinkDialog}
        onClose={() => setDiscordLinkDialog(false)}
