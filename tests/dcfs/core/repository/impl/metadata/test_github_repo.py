...
from dcfs.core.repository.impl.metadata.github_repo import GithubRepoMetadataRepository
from dcfs.core.repository.impl.metadata.github_repo.gh_directory import (
    GithubConfig,
    GithubDirectory,
)
...
class TestGithubRepoMetadataRepository:
    ...
    @patch("dcfs.core.repository.impl.metadata.github_repo.Github")
    ...
    async def test_get_metadata(self, mock_github_class, mock_github_config):
        ...
        # Should return DCFSMetadata with GithubDirectory
        assert isinstance(result, DCFSMetadata)
    ...
    @patch("dcfs.core.repository.impl.metadata.github_repo.Github")
    @patch("dcfs.core.repository.impl.metadata.github_repo.logger")
    ...
    async def test_push_method(self, mock_github_config):
        ...
        with patch("dcfs.core.repository.impl.metadata.github_repo.Github"):
            repository = GithubRepoMetadataRepository(mock_github_config)
...
class TestGithubDirectory:
    ...
    def test_init_with_explicit_values(self, mock_ghc):
        ...
        children: List[DCFSDirectory] = [Mock()]
        files: List[DCFSFileRef] = [Mock()]
        parent = Mock()
    ...
    def test_create_file_ref_success(self, mock_ghc):
        ...
        assert isinstance(file_ref, DCFSFileRef)
    ...
    def test_delete_file_ref_success(self, mock_ghc):
        ...
        from dcfs.core.model import DCFSFileRef

        file_ref = DCFSFileRef(message_id=12345, name="testfile", location=directory)
    ...
    @patch("dcfs.core.repository.impl.metadata.github_repo.gh_directory.logger")
...
class TestErrorHandling:
    ...
    def test_file_ref_with_non_numeric_message_id(self, mock_ghc):
        ...
        with patch("dcfs.core.repository.impl.metadata.github_repo.Github"):
            ...
            with patch(
                "dcfs.core.repository.impl.metadata.github_repo.logger"
            ) as mock_logger:
                root_dir = repository._build_directory_structure()
