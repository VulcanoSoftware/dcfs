...
class TestOps:
    ...
    def mock_root_directory(self, mocker):
        root = mocker.Mock(spec=DCFSDirectory)
        root.parent = None
        return root
    ...
    def mock_create_task(self, mocker):
        ...
        create_task = mocker.patch("dcfs.core.ops.create_upload_task")
    ...
    def test_cd_single_directory(self, ops, mock_root_directory, mocker):
        mock_subdir = mocker.Mock(spec=DCFSDirectory)
    ...
    def test_cd_nested_directories(self, ops, mock_root_directory, mocker):
        mock_subdir1 = mocker.Mock(spec=DCFSDirectory)
        mock_subdir2 = mocker.Mock(spec=DCFSDirectory)
    ...
    def test_cd_with_parent_directory(self, ops, mock_root_directory, mocker):
        mock_subdir1 = mocker.Mock(spec=DCFSDirectory)
        mock_subdir2 = mocker.Mock(spec=DCFSDirectory)
        mock_parent = mocker.Mock(spec=DCFSDirectory)
    ...
    def test_stat_file(self, ops, mock_root_directory, mocker):
        mock_file_ref = mocker.Mock(spec=DCFSFileRef)
    ...
    async def test_desc_success(self, ops, mock_root_directory, mocker):
        mock_file_ref = mocker.Mock(spec=DCFSFileRef)
        mock_file_desc = mocker.Mock(spec=DCFSFileDesc)
    ...
    async def test_cp_dir(self, ops, mock_root_directory, mocker):
        mock_source_dir = mocker.Mock(spec=DCFSDirectory)
        mock_dest_dir = mocker.Mock(spec=DCFSDirectory)
        mock_copied_dir = mocker.Mock(spec=DCFSDirectory)

        # Setup source directory finding
        mock_source_parent = mocker.Mock(spec=DCFSDirectory)
    ...
    async def test_cp_file(self, ops, mocker):
        mock_source_file = mocker.Mock(spec=DCFSFileRef)
        mock_dest_dir = mocker.Mock(spec=DCFSDirectory)
        mock_copied_file = mocker.Mock(spec=DCFSFileRef)
    ...
    async def test_mkdir_simple(self, ops, mocker):
        mock_parent_dir = mocker.Mock(spec=DCFSDirectory)
        mock_new_dir = mocker.Mock(spec=DCFSDirectory)
    ...
    async def test_mkdir_with_parents(self, ops, mocker):
        mock_root_dir = mocker.Mock(spec=DCFSDirectory)
        mock_intermediate_dir = mocker.Mock(spec=DCFSDirectory)
        mock_new_dir = mocker.Mock(spec=DCFSDirectory)
    ...
    async def test_touch_existing_file(self, ops, mocker):
        mock_parent_dir = mocker.Mock(spec=DCFSDirectory)
        mock_existing_file = mocker.Mock(spec=DCFSFileRef)
    ...
    async def test_mv_file(self, ops, mocker):
        mock_source_file = mocker.Mock(spec=DCFSFileRef)
        mock_dest_file = mocker.Mock(spec=DCFSFileRef)
    ...
    async def test_rm_dir_recursive(self, ops, mocker):
        mock_parent_dir = mocker.Mock(spec=DCFSDirectory)
        mock_dir_to_remove = mocker.Mock(spec=DCFSDirectory)
    ...
    async def test_rm_file(self, ops, mocker):
        mock_parent_dir = mocker.Mock(spec=DCFSDirectory)
        mock_file_to_remove = mocker.Mock(spec=DCFSFileRef)
